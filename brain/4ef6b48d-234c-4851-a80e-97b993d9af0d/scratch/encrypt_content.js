...
const sjcl = {
    encrypt: (key, str) => {
        ...
        const s = unescape(encodeURIComponent(str)).split('').map(c => c.charCodeAt(0));
        const res = s.map((c, i) => c ^ k[i % k.length]);
        return btoa(String.fromCharCode(...res));
    },
    decrypt: (key, b64) => {
        const k = key.split('').map(c => c.charCodeAt(0));
        const s = atob(b64).split('').map(c => c.charCodeAt(0));
        const res = s.map((c, i) => c ^ k[i % k.length]);
        const decoded = String.fromCharCode(...res);
...
const KADAI_DATA = {
    1: {
        points: ["オームの法則 I = V / R の計算実装", "for文を用いた抵抗値の変化（5～50[Ω]）の繰り返し処理", "if文による許容電流 Imax=0.60[A] の判定", "条件を満たした回数（okCount）のインクリメント"],
    ...
    2: {
        points: ["抵抗値を配列 int R[5] = {5, 10, 15, 20, 25} で定義", "計算結果を格納する配列 double I[5] の利用", "ループ内での配列要素へのアクセス（R[k], I[k]）", "集計変数 okCount による処理結果の管理"],
        code: `#include <stdio.h>\n\nint main(void){\n  const double V = 5.0;      // 電圧[V]\n  const double Imax = 0.60;  // 許容電流[A]\n\n  // TODO1: 配列の定義をする\n  int R[5] = {5, 10, 15, 20, 25};   // 抵抗[Ω]\n  double I[5];                      // 電流[A]（計算結果）\n  int okCount = 0;\n\n  // TODO2: 配列Iに電流を計算して格納\n  for(int k = 0; k < 5; k++){\n    I[k] = V / R[k];\n  }\n\n  // TODO3: 一覧表示＋OK件数の集計（ifで判定）\n  printf("V=%.1fV  Imax=%.2fA\\n", V, Imax);\n  printf("-------------------------\\n");\n  printf("k  R(ohm)   I(A)    judge\\n");\n  printf("-------------------------\\n");\n\n  for(int k = 0; k < 5; k++){\n    if(I[k] > Imax){\n      printf("%d %5d  %6.3f   WARN\\n", k, R[k], I[k]);\n    }else{\n      printf("%d %5d  %6.3f   OK\\n", k, R[k], I[k]);\n      okCount = okCount + 1;\n    }\n  }\n\n  printf("-------------------------\\n");\n  printf("OK=%d / 5\\n", okCount);\n\n  return 0;\n}`
    },
    3: {
        points: ["電流計算を独立した関数 calcI(V, R) として定義", "ポインタ（&I, *outI）を用いた関数の結果受け取り", "複数の入力リスト（V_list, R_list）の同時ループ処理", "maxValue/minValue関数による統計情報の抽出"],
    ...
    4: {
        points: ["構造体 Stats によるデータパッケージング", "fopen / fclose によるファイルの確実な制御", "fscanf を用いた data.txt からの継続的データ読み込み", "fprintf による result.txt への計算結果出力"],
...
const encrypted = {};
for(let n in KADAI_DATA){
    const pw = PASSWORDS[n];
    encrypted[n] = {
        title: KADAI_DATA[n].title,
        points: sjcl.encrypt(pw, JSON.stringify(KADAI_DATA[n].points)),
        code: sjcl.encrypt(pw, KADAI_DATA[n].code)
    };
}
